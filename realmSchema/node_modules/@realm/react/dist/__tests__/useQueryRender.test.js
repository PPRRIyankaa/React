...
var QueryType;
(function (QueryType) {
    QueryType[QueryType["filtered"] = 0] = "filtered";
    QueryType[QueryType["sorted"] = 1] = "sorted";
    QueryType[QueryType["normal"] = 2] = "normal";
})(QueryType || (QueryType = {}));
const App = ({ queryType = QueryType.normal, useUseObject = false }) => {
    return (React.createElement(RealmProvider, null,
        React.createElement(SetupComponent, null,
            React.createElement(View, { testID: "testContainer" },
                React.createElement(TestComponent, { queryType: queryType, useUseObject: useUseObject })))));
};
...
const ItemComponent = React.memo(({ item }) => {
    ...
    return (React.createElement(View, { testID: `result${item.id}` },
        React.createElement(View, { testID: `name${item.name}` },
...
const TagComponent = React.memo(({ tag }) => {
    tagRenderCounter();
    return (React.createElement(View, { testID: `tagResult${tag.id}` },
        React.createElement(View, { testID: `tagName${tag.id}` },
            React.createElement(Text, null, tag.name))));
});
const FILTER_ARGS = ["id < 20"];
...
function getTestCollection(queryType) {
    switch (queryType) {
        case QueryType.filtered:
...
const setupTest = async ({ queryType = QueryType.normal, useUseObject = false }) => {
    const renderResult = render(React.createElement(App, { queryType: queryType, useUseObject: useUseObject }));
...
`("useQueryRender: $queryTypeName", ({ queryType }) => {
    ...
    it("renders data in one render cycle per visible object in collection", async () => {
        const { getByTestId } = render(React.createElement(App, { queryType: queryType }));
